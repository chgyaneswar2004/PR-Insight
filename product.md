...
CodeWatch 是一款基于大语言模型（LLM）的智能代码评审工具，旨在通过自动化代码分析提高开发团队的代码质量和开发效率。它能够自动分析代码提交，生成详细的评审报告，并通过电子邮件通知相关人员。
...
CodeWatch 可以在代码提交时自动触发评审流程，通过 Git 钩子机制捕获提交事件，分析更改的代码，并生成评审报告。
...
CodeWatch 生成结构化的 Markdown 格式评审报告，包含：
...
CodeWatch 支持多种大语言模型，以满足不同的需求和预算：
...
```bash
# 评审最新提交
python run_codewatch_commit.py --verbose

# 评审特定提交
python run_codewatch_commit.py --commit <commit_hash> --verbose
```

#### 评估时间段内的提交

```bash
python run_codewatch.py eval "<author>" --start-date YYYY-MM-DD --end-date YYYY-MM-DD --include .py
```

#### 评估 GitHub PR

```bash
python run_codewatch.py pr "owner/repo" <pr_number>
```

### 4.4 配置选项

CodeWatch 提供多种配置选项，可以通过环境变量或命令行参数设置：
...
**Q: 如何处理大文件或大量文件的评审？**  
A: CodeWatch 会自动处理文件分割和批处理，但对于特别大的文件，可能需要增加超时设置或选择更快的模型。
