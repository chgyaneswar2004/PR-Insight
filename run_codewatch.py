...
from codewatch.retrievers import GithubRetriever, GitlabRetriever
from codewatch.utils.langchain_utils import load_model_by_name
from codewatch.utils.email_utils import send_report_email
from codewatch.utils.git_hooks import install_git_hooks
from codewatch.utils.git_log_analyzer import get_file_diffs_by_timeframe, get_commit_diff, CommitInfo
from codewatch.utils.code_evaluator import DiffEvaluator, generate_evaluation_markdown


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="CodeWatch - AI-powered code review tool")
    ...
    eval_parser.add_argument("--output", help="Report output path, defaults to codewatch_eval_<author>_<date>.md")
    ...
    commit_parser.add_argument("--output", help="Report output path, defaults to codewatch_commit_<hash>_<date>.md")
...
async def evaluate_developer_code(
    ...
    if not output_file:
        ...
        output_file = f"codewatch_eval_{author_slug}_{date_slug}.md"
    ...
    if email_addresses:
        subject = f"[CodeWatch] Code Evaluation Report for {author} ({start_date} to {end_date})"
...
def generate_full_report(repository_name, pull_request_number, email_addresses=None, platform="github", gitlab_url=None):
    ...
    with get_openai_callback() as cb:
        ...
        # Save report to file
        report_file = f"codewatch_pr_{pull_request_number}.md"
        ...
        if email_addresses:
            subject = f"[CodeWatch] Code Review for {repository_name} PR #{pull_request_number}: {retriever.pull_request.title}"
            sent = send_report_email(
                to_emails=email_addresses,
                subject=subject,
                markdown_content=report,
...
async def review_commit(
    ...
    if not output_file:
        date_slug = datetime.now().strftime("%Y%m%d")
        output_file = f"codewatch_commit_{commit_hash[:8]}_{date_slug}.md"
    ...
    # Send email report if addresses provided
    if email_addresses:
        subject = f"[CodeWatch] Code Review for Commit {commit_hash[:8]}"
...
def main():
    ...
    if args.command == "pr":
        ...
        if success:
            ...
            print("CodeWatch will now automatically review new commits.")
        ...
        print("Example: python run_codewatch.py pr owner/repo 123                      # GitHub PR review")
        print("Example: python run_codewatch.py pr owner/repo 123 --platform gitlab    # GitLab MR review")
        print("Example: python run_codewatch.py setup-hooks                           # Set up git hooks")
        print("Example: python run_codewatch.py eval username --start-date 2023-01-01 --end-date 2023-01-31  # Evaluate code")
        print("Example: python run_codewatch.py commit abc123def                      # Review local commit")
        print("Example: python run_codewatch.py commit abc123def --repo owner/repo --platform github  # Review GitHub commit")
        print("Example: python run_codewatch.py commit abc123def --repo owner/repo --platform gitlab  # Review GitLab commit")
