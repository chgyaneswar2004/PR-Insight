...
As a example, we will use codewatch to review a pull request on Github.
...
codewatch currently only supports python 3.10.
...
repository = "codewatch-ai/codewatch"
...
```python
from codewatch.chains import PRSummaryChain
...
We have a simple server demo to deploy codewatch as a service with fastapi and handle Github webhook.
Basicly you can also use it with workflow or Github Application.

see `examples/server.py`

Note that codewatch don't have fastapi and unicorn as dependency, you need to install them manually.
## Configuration
