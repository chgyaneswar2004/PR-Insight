...
# 导入 grimoire 模板
from codewatch.templates.grimoire_en import CODE_SUGGESTION
from codewatch.templates.grimoire_cn import GrimoireCn
# 导入优化的代码评审prompt
from codewatch.templates.optimized_code_review_prompt import (
    SYSTEM_PROMPT,
    CODE_REVIEW_PROMPT,
...
from codewatch.utils.git_log_analyzer import CommitInfo
...
"""
    ...
    async def evaluate_commit(
        ...
        for idx, (file_path, diff_info) in enumerate(commit_diff.items()):
            file_eval = await evaluate_single_file(file_path, diff_info, idx)
            file_evaluations.append(file_eval)
            # Spacing delay to respect API rate limits
...
def generate_commit_report_markdown(review_results: Dict[str, Any]) -> str:
    ...
    markdown = f"# CodeWatch Commit Review Report - {commit_hash[:8]}\n\n"
