...
class Settings(BaseModel):
    ...
    smtp_password: Optional[str] = Field(default_factory=lambda: os.environ.get("CODEWATCH_SMTP_PASSWORD") or os.environ.get("SMTP_PASSWORD"))
    ...
    max_concurrency: int = Field(default_factory=lambda: int(os.environ.get("CODEWATCH_MAX_CONCURRENCY", "1")))

    # Code Suffixes
    codewatch_support_code_file_suffix: str = Field(default_factory=lambda: os.environ.get("CODEWATCH_SUPPORT_CODE_FILE_SUFFIX", "py,java,go,js,ts,php,c,cpp,h,cs,rs"))
    codewatch_suffix_language_mapping: str = Field(default_factory=lambda: os.environ.get("CODEWATCH_SUFFIX_LANGUAGE_MAPPING", ""))
    ...
    def support_code_file_suffix_set(self) -> Set[str]:
        return set([ext.strip().lower() for ext in self.codewatch_support_code_file_suffix.split(",") if ext.strip()])
    ...
    def suffix_language_mapping_dict(self) -> Dict[str, str]:
        ...
        if self.codewatch_suffix_language_mapping:
            try:
                # Support JSON format or comma-separated "ext:lang" format
                if self.codewatch_suffix_language_mapping.strip().startswith("{"):
                    mapping.update(json.loads(self.codewatch_suffix_language_mapping))
                else:
                    for item in self.codewatch_suffix_language_mapping.split(","):
                        if ":" in item:
                            k, v = item.split(":", 1)
